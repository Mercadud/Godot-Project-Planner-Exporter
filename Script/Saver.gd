...
func exportProject():
	thread = Thread.new()
	thread.start(self, "_thread_function", nodeList)

func _thread_function(_welp):
	###OLD EXPORT METHOD###
...
#					file.store_string(nodeList[i]["name"] + "=\"*res:/" + nodeList[i]["path"].substr(resLoc, nodeList[i]["path"].length()) + "\"\n")
	...
	for node in get_tree().get_nodes_in_group("node"):
		if "RootFolder" in nodeList[node]["nodeName"]:
			node.updateInfo()
			break
	exportRootFolder()
	...
	for node in get_tree().get_nodes_in_group("node"):
		if "Script" in nodeList[node]["nodeName"]:
			node.updateInfo()
	exportScripts()
	
	#Add World Environments to array
	for node in get_tree().get_nodes_in_group("node"):
		if "WorldEnvironment" in nodeList[node]["nodeName"]:
			node.updateInfo()
	exportWorldEnvironment()
	
	#Add Scenes to array
	for node in get_tree().get_nodes_in_group("node"):
		if "Scene" in nodeList[node]["nodeName"]:
			node.updateInfo()
	exportScenes()
